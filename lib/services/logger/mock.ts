...
class MockLoggerService {
    ...
    async healResult(result: Types.IHealResult): Promise<void> {
        this.capture(
            ...
            `[HEAL] Operation completed: ${result.success}`,
            result
        );
    }
    ...
    printCapturedMessages(): void {
        ...
        this.messages.forEach((msg, index) => {
            ...
            console.log(`${index + 1}. [${levelName}] ${msg.message}`);
            if (msg.details) {
    ...
    private capture(level: Types.TLogLevelValue, message: string, details?: unknown): void {
...
class MockLoggerManager {
    ...
    getLogger(tag: Types.ILogTag, config?: Partial<Types.ILoggerConfig>): MockLoggerService {
        const key = `${tag.service}:${tag.target || 'default'}`;
        
        if (!this.loggers.has(key)) {
            const mergedConfig = { ...CONSTS.DEFAULT_LOG_CONFIG, ...this.globalConfig, ...config };
    ...
    clearAllMessages(): void {
        this.loggers.forEach(logger => logger.clearMessages());
    }
