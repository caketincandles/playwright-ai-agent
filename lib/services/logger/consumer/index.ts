...
export class Log {
    ...
    private formatOutput(details: Types.IConsumerLog): string {
        if (!details.changeLog && !details.recommendations) {
            return (
                chalk.dim(
                    `${details.filePath} has no changes or recommendations`,
                ) + '\n'
            );
        }

        const parts: string[] = [chalk.bold(details.filePath + ':'), '\n\n'];

        if (details.changeLog) {
            parts.push(chalk.bold.inverse('CHANGELOG'), '\n');
        ...
        if (details.recommendations) {
            parts.push(chalk.bold.inverse('RECOMMENDATIONS'), '\n');
    ...
    private get formatTag(): string {
        const str = this.config.target
            ? `${this.config.service}.${this.config.target}`
            : this.config.service;
